...
HEALTHY_THRESHOLD = 80  # SoH percentage threshold for healthy batteries
EXCHANGE_THRESHOLD = 63  # SoH percentage threshold for exchange batteries
...
def count_batteries_by_health(present_capacities):
    ...
    for capacity in present_capacities:
        soh_percentage = (capacity / RATED_CAPACITY) * 100  # Calculate State of Health (SoH) percentage
        
        if soh_percentage > HEALTHY_THRESHOLD:
            counts["healthy"] += 1
        elif HEALTHY_THRESHOLD >= soh_percentage >= EXCHANGE_THRESHOLD:
            counts["exchange"] += 1
        else:
            counts["failed"] += 1
